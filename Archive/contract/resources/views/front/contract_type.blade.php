...
<!-- Start Hero Area -->
    ...
    <section class="header-area">
        <div class="container">
            <h3 class="text-center">{{$subCategories->sub_categories_name}}</h3>
    ...
    <form class="form" method="post" id="contract_type" name="contract_type" data-parsley-validate action="{{ route('contract.saveContractType') }}" enctype="multipart/form-data">
                                    @csrf
        <div class="container">
            <!-- <div class="main-title">
                <h3>{{$subCategories->sub_categories_name}}</h3>
            </div> -->
            ...
            @if(!empty($contracts) && count($contracts) > 0)
                ...
                @foreach ($contracts as $contract)        
                    <label class="btn btn-outline-secondary form-check-label <?php if($i==1){echo "active";}?>">
                        <input class="form-check-input" type="radio" name="contract_type" value="{{$contract->id}}" id="{{$contract->id}}" autocomplete="off" <?php if($i==1){echo "checked";}?>>
                        {{ $contract->title }}
                    </label>
                    <?php $i++;?>
                    @endforeach
            ...
            @endif   
             ...
             @if(!empty($contracts) && count($contracts) > 0)
            <div class="button">
                <!-- <a href="#!" class="btn">Create My Document</a> -->
                <button type="submit" name="submit" value="submit" class="btn mt-2">Create My Contract</button>
            </div>
            @endif
        </div>
        </form>
